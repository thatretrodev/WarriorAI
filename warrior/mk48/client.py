...
class Client:
	...
	def __init__(self, metadata_endpoint: str = endpoints.ENDPOINT_META_WS, session_endpoint: str = endpoints.ENDPOINT_SESSION_WS):
		...
		async for msg in self.session_ws:
			...
			if 'terrain' in list(msg.keys()):
				...
				for c in tu.chunks:
					...
					c.get_image().save(f"chunks/{c.chunk[0]}_{c.chunk[1]}.png")
					#c.print()
					print('======================================')
				exit()
	...
	async def init_session(self):
		...
		self.session_ws = await websockets.connect(
			self.session_ws_endpoint.format(server_id=self.session.server_id, session_id=self.session.session_id)
		)
	...
	async def init_player(self, name: str = None):
		...
		# print('ok')
		self.session = protocol.SessionCreated.decode(await self.meta_ws.recv())
		# print('umm')
	...
	async def spawn(self, entity_type: str):
		await self.session_ws.send(protocol.Spawn(entity_type=entity_type).encode())
