...
def decode_message(cls, msg: str) -> Union[dict, Any]:
	decoded = json.loads(msg)[cls.__message__() if hasattr(cls, '__message__') else cls.__name__]
...
def message(cls=None, name=None):
	...
	_set_class_method(cls, staticmethod(lambda msg: decode_message(cls, msg)), name='decode')
	if name is not None:
...
class ChunkUpdate:
	...
	def altitudes(self):
		...
		if not self.is_update:
			self.cached_altitudes = [[0 for _ in range(CHUNK_SIZE)] for _ in range(CHUNK_SIZE)]
			# 0, 1
			# 2, 3
			# 4, 5
			# 6, 7
			bytes = rle.decode([b & 0xf for b in self.bytes], [(b & 0xf) + 1 for b in self.bytes])
	...
	def get_image(self):
		...
		for i in range(CHUNK_SIZE):
			for j in range(CHUNK_SIZE):
				try:
					altitude = self.cached_altitudes[i][j] << 4
				
					impix[i, j] = (altitude, altitude, altitude)
				except:
					impix[i, j] = (255, 0, 0)
				#print(shades[v], end='')
